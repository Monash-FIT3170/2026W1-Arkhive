 ...
 */
export const extractStructuredComponents = (pages) => {
    const rowMap = {};
    processParagraphsAndFillRowMap(pages, rowMap);
    // 3. Process Rows into Components
    const components = Object.keys(rowMap)
        .map(Number)
        .sort((a, b) => a - b)
 ...
 * @author Harsha Sharma (33879303)
 */
const flattenParagraphsToCellMap = (pages) => {
    return pages
 ...
 * @author Harsha Sharma (33879303)
 */
function postProcessIndentation(components) {
 ...
 * @author Harsha Sharma (33879303)
 */
const processParagraphsAndFillRowMap = (pages, rowMap) => {
    const Y_THRESHOLD = 15;
    const allParagraphs = pages.flatMap(page => (page.blocks ?? []).flatMap(block => block.paragraphs ?? []));
    allParagraphs.forEach(para => {
        ...
        const existingKey = Object.keys(rowMap).find(key => Math.abs(Number(key) - yCenter) < Y_THRESHOLD);
 ...
 * @author Harsha Sharma (33879303)
 */
const identifyTypeAndCreateComponents = (y, index, rowMap) => {
    const paragraphs = rowMap[y.toString()];
    // Sort paragraphs horizontally (Left to Right)
    const sortedParts = paragraphs.sort((a, b) => (a.boundingBox?.vertices?.[0]?.x ?? 0) - (b.boundingBox?.vertices?.[0]?.x ?? 0));
    // Extract text content
    const partTexts = sortedParts.map(p => p.words?.map(w => w.symbols?.map(s => s.text).join('')).join(' ') ?? "");
 ...
 * @author Harsha Sharma (33879303)
 */
const AddColumnsToRows = (comp, components) => {
    const table_cols = components[components.findIndex(c => c.type === "TABLE_COLS")];
    comp.type == "TABLE_ROW" ? Object.keys(comp.boundingBoxes).map((num1, _) => Object.keys(table_cols.boundingBoxes).map((num, _) => {
