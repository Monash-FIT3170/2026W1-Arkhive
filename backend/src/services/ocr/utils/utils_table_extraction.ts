...
function getComponents(rowMap: Record<string, google.cloud.vision.v1.IParagraph[]>){
  return Object.keys(rowMap)
      ...
      .map((y, index) => {
        ...
        const height = Math
        .abs((sortedParts[0]?.boundingBox?.vertices?.[3]?.y ?? 0) - (sortedParts[0]?.boundingBox?.vertices?.[0]?.y ?? 0));
...
function addParentAndAddColumns(components: OCRComponent[]) {
...
export const extractStructuredComponents = (pages: google.cloud.vision.v1.IPage[]): OCRComponent[] => {
  ...
  const components = getComponents(rowMap);
