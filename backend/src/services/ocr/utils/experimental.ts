...
/**
 * Incomplete module for manually calculating indentation level(may be removed later)
 * @param lines 
 * @param indentThresholdInInches 
 * @returns 
 * 
 * AI DECLARATION: USED GEMINI FO RTHIS
 */
export function calculateLineIndents(lines: DocumentLineOutput[], indentThresholdInInches = 0.15) {
  if (lines.length === 0) return [];

  // 1. Find the leftmost X-coordinate across all lines in this column to establish baseline (Level 0)
  const minX = Math.min(...lines.map(line => line.polygon![0]));

  return lines.map(line => {
    const lineLeftX = line.polygon![0]; // Top-left corner X
    const xOffset = lineLeftX - minX;

    // 2. Explicit rule: Hyphen at the start automatically bumps indent level
    const hasHyphen = line.content.trim().startsWith('-');

    // 3. Coordinate rule: If X is shifted right by threshold, calculate indent depth
    let indentLevel = 0;
    if (xOffset > indentThresholdInInches) {
      // Calculate depth (e.g. 1 indent step for every ~0.15 inches shift)
      indentLevel = Math.max(1, Math.round(xOffset / indentThresholdInInches));
    } else if (hasHyphen) {
      indentLevel = 1;
    }
...
const mapTablesToOCRComponents =
  ...
  async (
    ...
    const thing = OCRResponse.analyzeResult?.pages[0].lines!
