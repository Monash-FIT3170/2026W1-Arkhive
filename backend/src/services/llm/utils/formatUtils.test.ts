...
describe('formatUtils', () => {
  describe('profileColumnLocally', () => {
    ...
    it('should detect CURRENCY format', () => {
      ...
      const result = profileColumnLocally(samples);
      expect(result).toBe('^\\$?----+----\\d{1,3}(,\\d{3})*(\\.\\d{2})?$'.replace('----+----', '')); // Matches CURRENCY rule regex
    });
  ...
  describe('maskToRegex', () => {
    ...
    it('should return variable length decimal regex when isVariableLength is true', () => {
    ...
    it('should return wildcard .* for empty masks or unknown variable formats', () => {
      expect(maskToRegex('', false)).toBe('.*');
      expect(maskToRegex('  ', false)).toBe('.*');
      expect(maskToRegex('AAA-999', true)).toBe('.*');
    });
  });
});
