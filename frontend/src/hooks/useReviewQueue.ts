    ...
    (updates: { fieldId: string; newValue: string }[]) => {
      onPagesChange((prev) => {
        ...
        updates.forEach(({ fieldId, newValue }) => {
          ...
          const issue = flaggedIssues.find((i) => i.fieldId === fieldId && i.pageIndex === currentPageIndex);
      ...
      setFlaggedIssues((prev) => prev.filter((issue) => !(fieldIds.includes(issue.fieldId) && issue.pageIndex === currentPageIndex)));
    },
  ...
  const handleFetchBulkSuggestion = useCallback(
    async (
      ...
      const reviewFields: ReviewField[] = fields.map((f) => {
        const issue = flaggedIssues.find((i) => i.fieldId === f.fieldId && i.pageIndex === currentPageIndex);
        return {
          rowId: f.rowId,
          column,
          value: f.ocrValue,
