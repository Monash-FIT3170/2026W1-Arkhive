...
import { getProcessedImageUrls, getUploadedDocuments, getUploadedImageUrl } from '../../services/uploadService';
...
function ValidationPage() {
    ...
    async function loadSession() {
      try {
        ...
        try {
          ...
          groups = docs.map((doc, index) => ({
            fileIndex: index,
            fileName: doc.label || `Document ${index + 1}`,
            pageIndices: doc.pages.map((_, i) => i),
        ...
        // We still use these functions, but if the backend didn't tag fileIndex/pageIndex, they will just return all data
        const firstFileOcrData = getOcrDataForFile(ocrData, firstFile.fileIndex);
        const firstPageOcrData = getOcrDataForPage(firstFileOcrData, firstPageIndex);
        ...
        // Base logic for image URL
        let docImgUrl = processedUrls.length > 0 ? processedUrls[0] : (firstFile.documentId ? `/api/upload/image/${firstFile.documentId}/${firstPageIndex}` : '/api/upload/image');
        setDocumentImageURL(docImgUrl);
        
        // Base logic for flattened context, using flatten from flattener if present
  ...
  const showPageView = (fileIndex: number, pageIndex: number) => {
    ...
    setDocumentImageURL(docId !== undefined ? getUploadedImageUrl(docId, pageIndex) : getUploadedImageUrl());
    
    // Fallback to flattener flatten method if flattenOcrData is missing or wrong
    setDocumentContext(flatten(pageOcrData as OCRComponent[]));
    setHoveredDocumentOverlayId(null);
    setOldContext(null);
  };
